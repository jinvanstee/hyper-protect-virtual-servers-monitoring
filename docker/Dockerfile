...
#Export port
EXPOSE 9090
RUN mkdir -p /etc/prometheus/keys
COPY prometheus.yml /etc/prometheus/prometheus.yml
COPY keys /etc/prometheus/keys
COPY hosts /etc/hosts_temp
VOLUME [ "/prometheus" ]
ENTRYPOINT [ "/bin/sh", "-c", "cat /etc/hosts_temp >> /etc/hosts && exec /prometheus/prometheus --config.file=/etc/prometheus/prometheus.yml --web.console.libraries=/etc/prometheus/console_libraries --web.console.templates=/etc/prometheus/consoles" ]
#End of dockerfile
